...
class Counter:
        ...
        try:
            while True:
                ...
                if _data:
                    ...
                    #_buffer += _data
                    #print  pretty_hex(_buffer)
                    #print len(_buffer),_data, _buffer[-2:], str(bytearray(self._EOL))
                    #if getflag==1:
                    #    if len(_buffer)>2 and _buffer[-2:] == str(bytearray(self._EOL)):
                    #        break
                    #else:
                    #    if len(_buffer)>2 and _buffer[-1:] == str(bytearray(self._ETX)):
                        #print 'next command'
                    #        break
    ...
    def getValue(self, answer):
        ...
        #except:
        #    value=answer  
        #value = map(f, re.findall('(\d+.)', answer))
        value=answer
        if len(value) == 1:
            value = value[0]
        return value
...
# print 'T1 = {0[0]} Квт/ч\r\nT2 = {0[1]} Квт/ч'.format(en.cmd('ET0PE()'))
